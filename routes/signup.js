...
(req, res, next) => {
  ...
  if(usernameErrors.length > 0 || mailErrors.length > 0 || passErrors.length > 0){
    res.render("signup", {usernameErrors: usernameErrors, mailErrors: mailErrors, passErrors: passErrors});
  } else {
    next();
  }
},
(req, res, next) => {
  ...
  connection.query(sql, email, (error, results) => {
    ...
    } else if (results[0] != null) { //email登録済みの処理
      mailErrors.push("入力されたメールアドレスは登録済みです。");
    }
     if(mailErrors.length > 0){//エラーがあったら
      res.render("signup", {usernameErrors: [], mailErrors: mailErrors, passErrors: []});
    } else {
